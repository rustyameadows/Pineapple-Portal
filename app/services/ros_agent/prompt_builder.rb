module RosAgent
  class PromptBuilder
    ...
    DRAFT_DETAIL_INSTRUCTIONS = [
      ...
      "Use time_caption for display labels like All day, EOD, TBD, or timing coming soon.",
      "time_caption is not a substitute for starts_at; still provide the best available starts_at date or datetime when known.",
      "Do not use time_caption for normal exact clock times like 9:00 AM; use timing.starts_at for exact clock times.",
      "Use time_caption only when the source timing is a label/window rather than an exact clock time.",
